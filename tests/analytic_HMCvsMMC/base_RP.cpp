...
int main()
{
    ...
    Geom24 G11(p, q, dim, g2);
    Geom24 G12(p, q, dim, g2);
    ...
    // dual averaging hmc
    double dt_min = 0.005;
    G11.HMC(100, dt_min, 1000, engine, 0.9);
    double dt_max = 0.005;
    G12.HMC(100, dt_max, 1000, engine, 0.4);

    if(dt_min > dt_max)
    {
        cerr << "dt_min larger than dt_max" << endl;
        cerr << "dt_min: " << dt_min << endl;
        cerr << "dt_max: " << dt_max << endl;
        return 1;
    }

    // thermalization hmc_rp
    G1.HMC_RP(50, 150, dt_min, dt_max, 1000, engine);
    
    // dual averaging + thermalization mmc
    double scale = 0.128;
    G2.MMC(scale, 1000, engine, 0.232);

    ofstream out_s_h, out_s_m;
    out_s_h.open("data/" + filename_from_data(p, q, dim, g2, "HMC") + "_S_RP.txt");
    out_s_m.open("data/" + filename_from_data(p, q, dim, g2, "MMC") + "_S_RP.txt");

    // simulation
    clock_t start1 = clock();
    double ar_h = G1.HMC_RP(50, 150, dt_min, dt_max, iter, 1, engine, out_s_h);
    ...
    in_s_h.open("data/" + filename_from_data(p, q, dim, g2, "HMC") + "_S_RP.txt");
    in_s_m.open("data/" + filename_from_data(p, q, dim, g2, "MMC") + "_S_RP.txt");
    ...
    ofstream out_s;
    out_s.open("data/" + filename_from_data(p, q, dim, g2, "") + "_dofs_RP.txt");
